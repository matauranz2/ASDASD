...
# Step 3: Select the required data (all available rows)
filtered_data <- data %>%
  select(speed_of_sound) %>%  # Ensure correct column name
  filter(!is.na(speed_of_sound) & speed_of_sound != 0)  # Remove NA and zero values
...
# Step 7: Compute and print the harmonic mean dynamically
num_rows <- nrow(filtered_data)  # Count the number of valid rows
if (num_rows > 0) {
  harmonic_mean <- num_rows / reciprocal_sum
  cat("\nHarmonic Mean of Speed of Sound:", formatC(harmonic_mean, format = "f", digits = 7), "\n")
} else {
  cat("\nNo valid speed_of_sound values available to compute harmonic mean.\n")
}
plot(data)